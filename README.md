...
<br>
<b>如果你當初是用 git clone 下載的</b>（推薦）：**每次雙擊 <code>run.bat</code> 就會自動檢查更新**，不需另外動作。<br>
&nbsp;&nbsp;① 自動 <code>git pull</code>（失敗會跳過）<br>
&nbsp;&nbsp;② 偵測 requirements.txt / package.json 有改才重裝套件<br>
&nbsp;&nbsp;③ 自動重 build 前端<br>
&nbsp;&nbsp;④ 啟動後端 + 開瀏覽器<br>
你的 <code>backend/data/stocks.db</code> 完全不會動到。<br>
<br>
<b>如果你當初是下載 ZIP 的</b>：重下新 ZIP 解壓到旁邊新資料夾，把舊資料夾的 <code>backend/data/stocks.db</code> 複製進去，再跑 <code>setup.bat</code>。以後建議改用 git clone 才能享受自動更新。
</details>
